...
RSpec.describe API::V1::Todos::Item::UncompleteController, type: :controller do
  before do
    warden = double
  ...
  describe '#update' do
    context 'when the todo is found' do
      ...
      let(:todo) { create(:todo_item, :completed, user:) }
    ...
    context 'when something went wrong' do
      ...
      before do
        ...
        allow(Todo::Item::Uncomplete).to receive(:call).and_return(result)
      end
      ...
      it 'raises an error' do
        ...
        expect(Todo::Item::Uncomplete).to have_received(:call).with(->(input) {
          id = input[:scope].id
